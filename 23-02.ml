...
(* 目的 : ヒープを用いて大きい順にソートを行う *)
(* heap_sort : int list -> int list *)
let heap_sort lst =
  let he = Heap.create (List.length lst) 0 ""
  in
  let rec heap_insert lst heap =
    match lst with
        [] -> ()
      | first :: rest ->
      let (_,h) = Heap.insert heap first "" in
      heap_insert rest h in
  let rec h_s heap acc = 
    if Heap.length heap = 0 then acc
    else
      let (min, rest) = Heap.split_top heap in
      h_s rest ((fst min) :: acc)

  in heap_insert lst he;
  h_s he []
    
(* テスト *)

let test1 = heap_sort [5] = [5]
let test2 = heap_sort [1;2] = [2;1]
let test3 = heap_sort [2;1] = [2;1]
let test4 = heap_sort [9;7;2;5;6;8] = [9;8;7;6;5;2]